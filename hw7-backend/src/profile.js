...
const getHeadlines = (req, res) => {
	...
	Profile.find({username: {$in:users}}).exec((err,profiles) => {
        if(!profiles || profiles.length === 0 ){
            res.status(401).send('No headlines found')
            return
        }
        const result = []
...
const putHeadline = (req, res) => {
    ...
    if(!req.body.headline){
        res.status(401).send({errMsg:"missing the headline input"})
        return
    }
...
const putAvatar = (req, res) => {
    ...
    Profile.update({username:user},{$set:{avatar:req.body.avatar}}, (err, tank) => {
        if(err) {
            return console.log(err)
		} else {
            Profile.find({username:user}).exec((err,profiles) => {
