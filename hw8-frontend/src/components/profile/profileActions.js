import { ERRORMSG,SUCCESSMSG, UPDATE_PROFILE, FETCH_PROFILE, UPDATE_AVATAR, resource } from '../../actions'
...
/*
	Link account with regular account
*/
export const linkAccount = (e) => {
	return (dispatch) => {
		resource('POST', 'link', e).then(r => {
			if (r.result == 'success') {
				dispatch({
					type: SUCCESSMSG,
					successMsg: 'Link succeed'
				})
			} else {
				dispatch({
					type: ERRORMSG,
...
export const unlinkAccount = (e) => {
	return (dispatch) => {
		resource('POST', 'unlink', e).then(r => {
			if (r.result == 'success') {
				dispatch({
					type: SUCCESSMSG,
					successMsg: 'unLink succeed'
				})
			} else {
				dispatch({
					type: ERRORMSG,
					errorMsg: 'unLink failed'
				})
			}
...
/*
	Fetching profile from backend, including headline, email, zip, dob, avatar
*/
...
/*
	update profile to backend, once the updating succeed, the function will return success msg
*/
