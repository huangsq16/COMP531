  ...
  return (
    ...
    <div className="col-sm-4 col-sm-offset-4 card">
      <h1 className = "txt-align-center">Sign up</h1>
        <div>
            <input className = "commoninput" id="register_displayName" type="text" ref={(node) => displayName = node} placeholder="Display name"/>
        </div> 
        <div>
            <input className = "commoninput" id="register_email" type="text" ref={(node) => email = node} placeholder="Email address"/>
        </div>    
        <div>
            <input className = "commoninput" id="register_phone" type="text" ref={(node) => phone = node} placeholder="Phone number"/>
        </div> 
        <div>
            <input className = "commoninput" id="register_zip" type="text" ref={(node) => zip = node} placeholder="Zip code"/>
        </div> 
        <div>
            <input className = "commoninput" id="register_birthDate" type="text" ref={(node) => birthDate = node} placeholder="Birth date: mm/dd/yyyy"/>
        </div>
        <div>
            <input className = "commoninput" id="register_password" type="password" ref={(node) => password = node} placeholder="Password"/>
        </div> 
        <div>
          <input className = "commoninput" id="register_confirmPassword" type="password" ref={(node) => confirmPassword = node} placeholder="Confirm password"/>
        </div>
        <div>
        <button id='register_submit' onClick={_submit} className="btn btn-primary pure-button">Submit</button>
        </div>
      <div>
