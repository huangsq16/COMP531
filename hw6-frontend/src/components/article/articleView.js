        	...
        	<div key={_id + Math.random() * 100} name='totalArticles'>
		        <div className='panel panel-white post panel-shadow'>
		            <div className='post-heading'>
		                <div className='pull-left image'><img src={_avatar} className='img-circle avatar' alt='user profile image'/></div>
		                ...
		                {article.author == username && <button id={testupdateid} type='button' className='btn article-edit' onClick=
		                {e => { _updateArticle({text: _text, articleId: _id})}}><span className='glyphicon glyphicon-pencil' /></button>}
		            </div> 
		            {article.author != username && <div className='post-description'>
		            {article.img && <img id='img1' className='img_responsive' src={article.img} alt='Image'/> }<p>{article.text}</p></div>}
		            {article.author == username && <div className='post-description'>{article.img && 
		            <img id='img1' className='img_responsive' src={article.img} alt='Image'/> }
		            ...
		            <div className='post-footer'>
		                <form> 
		                	<div className='commentinline'>
			                    ...
			                    <button type='button' className='btn btn-default submit' onClick=
			                    {e => _updateComment({text: _comment.value, commentId: -1, articleId: _id})}>
