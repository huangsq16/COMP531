...
function RushBall() {
	...
	function addEvent() {
		...
		window.onclick = function(evt) {
			...
			if (x >= (canvas.width / 2 - 80 / 2) && x <= (canvas.width / 2 + 80 / 2) && y >= (canvas.height / 2 - 80 / 2) 
	...
	function createMagicCoins(magiccoins) {
		var tly = Math.round(Math.random() * 100);
		tly = tly + 100;
		var tlx = Math.round(Math.random() * maxgap);
	...
	function drawReplayIcon() {
		...
		ctx.drawImage(replayicon, canvas.width / 2 - 80 / 2,
	        canvas.height / 2 - 80 / 2, 80, 80);
		ctx.font = '24px serif';
		var yourscore = localStorage.getItem("curscore");
		ctx.strokeText(`your score: ${yourscore}`, 240, 74);
	...
	function draw() {
		...
		// do restore operation if ball collides obstacle
		if (over) {
			window.cancelAnimationFrame(raf);
			localStorage.setItem('highestscore', highest_score);
			localStorage.setItem('curscore', score);
			restoreGame();
			drawReplayIcon();
			return;
		} else {
